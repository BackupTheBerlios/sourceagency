...
while (is_array($HTTP_POST_VARS) 
    ...
    case "register": // Register a new user
        ...
        if ($user_type == "devel") {
          ...
          $message = $t->translate("Thank you for registering on the $sys_name Site. In order")."\n"
                     .$t->translate("to complete your registration, the $sys_name crew will contact you soon via this email address").".\n\n"
                     .$t->translate(" -- the $sys_name crew")."\n";
        }
        ...
        mail($email_usr,"[$sys_name] ".$t->translate("Registration as Sponsor in BerliOS SourceAgency"),$message,"From: $ml_newsfromaddr\nReply-To: $ml_newsreplyaddr\nX-Mailer: PHP");
          $msg = $t->translate("Congratulations")."! "
                .$t->translate("You have registered on $sys_name")."."
                ."<p>".$t->translate("Your new username is").": <b>$username</b>"
		."<p>".$t->translate("You are now being sent further information via email.").".";
 	      }
		if ($ml_notify) {
			$message  = "Username: $username\n";
                        if ($user_type == "devel") $message .= "User type: Developer\n";
                        else $message .= "User type: Sponsor\n";
...
<td align=right><?php echo $t->translate("Type") ?>:</td><td><input type="radio" name="user_type" value="devel" checked>Developer <input type="radio" name="user_type" value="sponsor">Sponsor</td>
</tr>
