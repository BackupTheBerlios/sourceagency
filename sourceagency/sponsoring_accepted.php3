...
if (check_proid($proid)) {
  ...
  if ($db->affected_rows()) {
	print "Sponsoring involvement by <b>$sponsor</b> has been accepted.<p>\n";
	htmlp_link("sponsoring.php3",array("proid" => $proid),"Back to the Sponsoring Involvement Page");
  } else {
  ...
  include("config.inc");
  monitor_mail($proid,"sponsoring", "Sponsoring involvement accepted for project $proid", "An event has happened.");

	// mail new sponsor that he has been accepted
  $db->query("SELECT email_usr FROM auth_user WHERE username='$sponsor'");
  $db->next_record();
  $message = "\nHi,$sponsor\n\nYour sponsoring involvement for project $proid has been accepted.\n\n";
  mail($db->f("email_usr"),"[".$GLOBALS["sys_name"]."] Sponsoring involvement accepted", $message,"From: ".$GLOBALS["ml_fromaddr"]."\nReply-To: ".$GLOBALS["ml_replyaddr"]."\nX-Mailer: PHP");  

	// Insert into history
  $db->query("INSERT history SET proid='$proid', history_user='".$auth->auth["uname"]."', type='sponsoring', action='Sponsor $sponsor accepted as sponsor'");
